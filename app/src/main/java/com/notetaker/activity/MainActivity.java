package com.notetaker.activity;
...
import com.notetaker.data.dao.DAOInterface;
import com.notetaker.widget.AboutNoticeDialog;
import com.notetaker.widget.NotesAdapter;
...
public class MainActivity extends RoboActionBarActivity {
    ...
    @Inject private DAOInterface DAOInterface;
        ...
        switch (item.getItemId()) {
            case R.id.action_about_info:
                new AboutNoticeDialog()
                        .show(getSupportFragmentManager(), "custom_dialog_about");
                return true;
    ...
    private void setupNotesAdapter() {
        ...
        for (Note note : DAOInterface.fetchAll()) {
    ...
    private void addNote(Intent data) {
        Note note = EditActivity.getExtraNote(data);
        DAOInterface.insert(note);
        NotesAdapter.NoteViewWrapper noteViewWrapper = new NotesAdapter.NoteViewWrapper(note);
        notesData.add(noteViewWrapper);
        updateView();
    ...
    private void deleteNotes(ArrayList<Integer> selectedPositions) {
        ...
        for (int position : selectedPositions) {
            ...
            DAOInterface.delete(noteViewWrapper.getNote());
        }
    ...
    private void updateNote(Intent data) {
        Note updatedNote = ViewActivity.getExtraUpdatedNote(data);
        DAOInterface.update(updatedNote);
    ...
    private void setListOnItemClickListenersWhenNoActionMode() {
        listView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                startActivityForResult(ViewActivity.buildIntent(MainActivity.this, notesData.get(position).getNote()), EDIT_NOTE_RESULT_CODE);
            }
        });
